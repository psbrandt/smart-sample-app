...
var App = (function() {
  ...
  App.prototype.loadResource = function(client, type, id, container) {
    if (type && id) {
      return client.api.read({ type: type, id: id }).then(
        function(result) {
          return renderJSON(container, result.data);
        },
        function(result) {
          container
            .text(
  ...
  App.prototype.loadPatient = function(client) {
    ...
    if (!this.scope.has("patient/*.read") && !this.scope.has("patient/*.*")) {
      if (this.options.patient.hidden === "auto") {
        return;
      }
      $(".patient")
        .text("No patient read access requested in scopes")
        .showPanel("has-error");
    } else {
  ...
  App.prototype.loadEncounter = function(client) {
    ...
    if (!client.tokenResponse.encounter) {
      ...
      $(".encounter")
        .text("No encounter in context")
        .showPanel();
    }
    if (
      this.scope.has("patient/Encounter.read") ||
      ...
      this.loadResource(
        client,
        "Encounter",
        client.tokenResponse.encounter,
        $(".encounter")
      );
    } else {
      if (this.options.encounter.hidden === "auto") {
      ...
      $(".encounter")
        .text("No encounter read access requested in scopes")
        .showPanel();
    }
  };
  App.prototype.loadUser = function(client) {
    ...
    if (this.options.user.hidden !== true) {
      if (tokens.length == 2) {
        if (this.scope.has("user/*.read") || this.scope.has("user/*.*")) {
          this.loadResource(client, tokens[0], tokens[1], $(".user"));
        } else {
          $(".user")
            .text("No user read access requested in scopes")
            .showPanel();
        }
      } else if (this.options.user.hidden === false) {
        $(".user")
          .text("No user in context")
          .showPanel();
      }
    }
  };
  ...
  App.prototype.getScopesFromForm = function() {
    var scope = $(":checkbox[name=scope]:checked")
      .get()
      .map(function(cb) {
  ...
  App.prototype.renderGlobalError = function(message) {
    ...
    if (!message) {
      message = [this.query.error, this.query.error_description]
        .filter(Boolean)
        .join("<br/>");
    }
    if (message) {
  ...
  App.prototype.renderToken = function(client, type) {
    ...
    if (token) {
      ...
      if (options.hidden === false) {
        pre
          .text("No " + type + " found in the authorization response")
          .showPanel();
      }
    }
  };
...
$(function() {
  var app = new App({
    ...
    scope:
      "patient/*.* user/*.* launch/patient launch/encounter openid offline_access profile"
  });
  app.init();
});
